...
const DessertList = () => {
    ...
    const[desserts,setDesserts]=useState("Ürünler")

  const array = [

        { name: 'Muhallebili Kadayıf', id: 0 },
        { name: 'Soğuk Baklava', id: 1 },
        { name: 'Ekler', id: 2 },
    ...
    return (
        <div>
            ...
            <div className="btn-group mt-3 w-100">
                ...
                {
                array.map((ar) => (
                    <button onClick={()=> setDesserts(ar.name)} key={ar.id} className="dropdown-item" type="button">{ar.name}</button>
                ))
            }
                </div>
